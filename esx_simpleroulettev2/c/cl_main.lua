...
Citizen.CreateThread(function()
	...
	local coords = GetEntityCoords(PlayerPedId())
		if (GetDistanceBetweenCoords(coords, coordx, coordy, coordz, true) < 5) then
			sleep = false
			ESX.ShowHelpNotification('Paina ~INPUT_PICKUP~ pelataksesi ~g~Rulettia')
...
function bettaus22()
	maxLength = 5
	AddTextEntry('FMMC_KEY_TIP8', "Aseta ~y~Panos")
	...
	local elements = {
		{ label = '<span style="color:red;">Punainen</span>', action = 'Punainen' },
		{ label = '<span style="color:black;">Musta</span>', action = 'Musta' },
		{ label = '<span style="color:limegreen;">Vihreä</span>', action = 'Vihreä' },
	}
	...
	function(data, menu)
		...
		if colour == "Punainen" or colour == 'Musta' or colour == 'Vihreä' then
...
function DrawText3D(x,y,z, ruletti)
  ...
  if onScreen then
		...
		SetTextEntry("STRING")
		SetTextCentre(1)
		AddTextComponentString('~w~Pallo ~g~pyörii:~n~~r~ ' ..(ruletti[math.random(1, #ruletti)])..'.')
		DrawText(_x,_y)
    end
end
